...
$customer          = $order instanceof WC_Order ? $order->get_meta( '_customer_user' ) : 0;
		...
		foreach ( $raq_data as $key => $field ) {
			...
			switch ( $field_type ) {
				...
				case 'country':
					...
					?>
					<div>
						<strong><?php echo esc_html( $field['label'] ); ?></strong>: <?php echo esc_html( $countries[ $country_code ] ); ?>
					</div>
					<?php
					break;
