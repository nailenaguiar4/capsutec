...
if ( ! class_exists( 'YWRAQ_Gutenberg' ) ) {
	...
	class YWRAQ_Gutenberg {
		...
		/**
		 * Remove patterns from other categories
		 *
		 * @return void
		 */
		protected function remove_pattern_from_other_categories() {
			$template_list = WP_Block_Patterns_Registry::get_instance()->get_all_registered();
			if ( $template_list ) {
				foreach ( $template_list as $template ) {
					if ( isset( $template['categories'] ) && ! in_array( 'yith-template-pdf-pattern', $template['categories'], true ) ) {
						WP_Block_Patterns_Registry::get_instance()->unregister( $template['name'] );
					}
				}
			}
		}
		...
		public function gutenberg_integration() {
			...
			if ( ywraq_check_valid_admin_page( YITH_YWRAQ_Post_Types::$pdf_template ) ) {
				...
				wp_register_script( 'ywraq-pdf-template-builder-script', YITH_YWRAQ_URL . 'dist/blocks/index.js', array( 'wp-blocks', 'wp-element', 'wp-editor', 'wp-components', 'wp-plugins', 'wp-i18n', 'wc-components' ), $version, false );
				wp_localize_script(
